...
partition = sci.loadmat('data/partition_VENICE.mat')['partition']
predictions = []
...
for index, test_index in enumerate(partition):
    ...
    json_file = open('model/model' + str(index) + '_VENICE.json', 'r')
    loaded_model_json = json_file.read()
    json_file.close()
    model = model_from_json(loaded_model_json)
    # load weights into new model
    model.load_weights("model/model" + str(index) + "_VENICE.h5")
...
sci.savemat('data/predictions_VENICE.mat', {'predictions':predictions})
